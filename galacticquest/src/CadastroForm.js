...
const CadastroForm = () => {
  ...
  const redirectToNextStep = (user) => {
    ...
    if (!user.grupo.naveEscolhida) {
      navigate("/SelecaoNave");
      return;
    }
    if (!user.grupo.equipeEscolhida) {
  ...
  const handleRegisterSubmit = async (e) => {
    ...
    if (!emailAvailable && !isUpdateMode) {
      alert("❌ Por favor, utilize um e-mail diferente. Este já está em uso.");
      return;
    }
    ...
    try {
      ...
      if (isUpdateMode) {
        ...
        if (response.data.success) {
          alert("✅ Dados atualizados com sucesso! Faça login para continuar.");
      ...
      alert(`❌ Erro ao processar o registro/atualização!\n\n${error?.response?.data?.details || error.message}`);
    }
  };
      ...
      */}
      <div className="language-selector">
        <img
          ...
          alt="Português"
          className={`flag ${selectedLanguage === 'brazil' ? 'active' : ''}`}
          onClick={() => handleLanguageChange('pt', 'brazil')}
        />
        <img
          src="/images/flags/spain.png"
          alt="Español"
          className={`flag ${selectedLanguage === 'spain' ? 'active' : ''}`}
          onClick={() => handleLanguageChange('es', 'spain')}
        />
        <img
          src="/images/flags/usa.png"
          alt="English"
          className={`flag ${selectedLanguage === 'usa' ? 'active' : ''}`}
          onClick={() => handleLanguageChange('en', 'usa')}
        />
        <img
          src="/images/flags/italy.png"
          alt="Italiano"
          className={`flag ${selectedLanguage === 'italy' ? 'active' : ''}`}
      ...
      <div className="form-container">
        {!isRegistering ? (
          ...
          <form onSubmit={handleRegisterSubmit} className="cadastro-form">
            ...
            />
            {isUpdateMode && (
              <p style={{ color: 'yellow', fontSize: '12px', margin: '-5px 0 5px 0' }}>
                E-mail não pode ser alterado. Você está no modo de atualização.
              </p>
            )}
            <input
              type="text"
            ...
            {!emailAvailable && emailChecked && !isUpdateMode && (
              <p style={{ color: 'red', fontSize: '12px', margin: '-5px 0 5px 0' }}>
                {t('emailInUse')}
              </p>
            )}
            ...
            <p onClick={() => setIsRegistering(false)} className="toggle-form">
              {t('toggleToLogin')}
            </p>
          </form>
        )}
