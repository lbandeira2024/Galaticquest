...
// Constantes e dados estáticos (permanecem inalterados)
...
// Lista de corpos com água (baseado na sua lista, incluindo Terra e Proxima Centauri b)
const hasWaterList = new Set([
    ...
    "Terra", "Proxima Centauri b" // Adicionados com base na sua lista
]);
...
const realDistances = {
    ...
    "Delfos": 2875, "Mol": 4500, "Skylab": 115, "Almaz": 6000, "Tiangong": 1500
};
...
const StellarMapPlan = ({ onRouteComplete, onRouteReset }) => {
    ...
    useEffect(() => {
        ...
        const fetchInitialData = async () => {
            try {
                const response = await fetch(`${API_BASE_URL}/${userId}/game-data`);
                const data = await response.json();
                if (data.success) {
                    if (data.rotaPlanejada && data.rotaPlanejada.length > 0) {
                        dispatchRouteAction({ type: 'SET_SAVED_ROUTE', payload: { steps: data.rotaPlanejada } });
                        setIsRouteConfirmed(true);
                    }
                    if (data.routeIndex !== undefined) {
                        setCurrentRouteIndex(data.routeIndex);
                    }
                }
            } catch (error) {
                console.error("StellarMapPlan: Erro ao buscar dados iniciais do usuário:", error);
            }
        };
        fetchInitialData();
    }, [userId, API_BASE_URL]);
    ...
    useEffect(() => {
        if (isRouteConfirmed || !userId || !API_BASE_URL) return;
        const fetchLatestProgress = async () => {
            setIsLoadingProgress(true);
            try {
                const response = await fetch(`${API_BASE_URL}/${userId}/game-data`);
                const data = await response.json();
                if (data.success && data.routeIndex !== undefined) {
                    setCurrentRouteIndex(data.routeIndex);
                }
            } catch (error) {
                console.error("StellarMapPlan: Erro ao re-buscar progresso da rota:", error);
            } finally {
                setIsLoadingProgress(false);
            }
        };
        fetchLatestProgress();
    }, [isRouteConfirmed, userId, API_BASE_URL]);
    ...
    const visitedDestinations = useMemo(() =>
        plannedRoute.steps.slice(0, currentRouteIndex + 1).map(step => step.name),
        [plannedRoute.steps, currentRouteIndex]
    );

    const nextDestination = useMemo(() =>
        (currentRouteIndex + 1 < plannedRoute.steps.length) ? plannedRoute.steps[currentRouteIndex + 1].name : null,
        [plannedRoute.steps, currentRouteIndex]
    );
    ...
    const handleRemoveStep = useCallback((index) => {
        if (index <= currentRouteIndex + 1) return;
        dispatchRouteAction({ type: 'REMOVE_STEP', payload: { index } });
    }, [currentRouteIndex]);

    const handleMoveStep = useCallback((index, direction) => {
        if (index <= currentRouteIndex + 1) return;
        if (direction === 'up' && index === currentRouteIndex + 2) return;
        dispatchRouteAction({ type: 'REORDER_STEP', payload: { index, direction } });
    }, [currentRouteIndex]);

    const savePlannedRoute = useCallback(async (routeToSave) => {
        if (!userId || !API_BASE_URL) return;
        try {
            await fetch(`${API_BASE_URL}/save-planned-route`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ userId, routeSteps: routeToSave.steps }),
            });
        } catch (error) {
            console.error("Erro de rede ao salvar rota planejada:", error);
        }
    }, [userId, API_BASE_URL]);
    ...
    const getWarning = (value, max, type) => {
        ...
        if (value >= max * 0.6) return `Atenção: Uso de ${type} elevado`;
        return null;
    };
    ...
    const isRouteInProgress = currentRouteIndex > 0;
                ...
                <div className="route-display-panel ultimate">
                    <div className="actions-ultimate">
                        ...
                        <button
                            ...
                            disabled={isRouteConfirmed || isRouteInProgress}
                        >
                    ...
                    <div className="resources-ultimate">
                        <div className="resource-ultimate fuel-ultimate">
                            <img src="/images/plan/Combustivel-port.png" alt="Combustível" className="resource-icon-ultimate" />
                        ...
                        <div className="resource-ultimate oxygen-ultimate">
                            <img src="/images/plan/Oxigenio-port.png" alt="Oxigênio" className="resource-icon-ultimate" />
                        ...
                        <div className="resource-ultimate food-ultimate">
                            <img src="/images/plan/Comida-port.png" alt="Provisões" className="resource-icon-ultimate" />
                    ...
                    <div className="route-container">
                        <div className="route-ultimate">
                            {plannedRoute.steps.map((step, index) => {
                                const markerClass = index <= currentRouteIndex ? "origin-marker"
                                    : index === currentRouteIndex + 1 ? "next-destination-marker"
                                        : "normal-marker";

                                const isStepLocked = index <= currentRouteIndex + 1;

                                // ALTERAÇÃO AQUI: Adiciona emoji se houver água
                                ...
                                return (
                                    <div key={index} className="step-ultimate" onMouseEnter={() => !isRouteConfirmed && setHoveredStep(index)} onMouseLeave={() => !isRouteConfirmed && setHoveredStep(null)}>
                                        ...
                                        <div className="step-content">
                                            {/* ALTERAÇÃO AQUI: Usa displayName */}
                                        ...
                                        {hoveredStep === index && !isRouteConfirmed && index > 0 && (
                                            <div className="step-move-controls">
                                                <button className="move-step-button up" onClick={() => handleMoveStep(index, 'up')} disabled={isLoadingProgress || isStepLocked || index === currentRouteIndex + 2}></button>
                        ...
                        <React.Fragment key={index}>
                            ...
                            {(planet.radius > 0 || planet.isStation) && (
                                <div className={`celestial-body ${planet.isStation ? 'station' : 'planet'}
                                    ...
                                    {/* Ícone de Água REMOVIDO DAQUI */}

                                    {/* Etiqueta do Planeta (agora posicionada acima) */}
                                    {/* Container da Label movido para fora para facilitar posicionamento */}
                                    ...
                                    {planet.moons.map((moon, moonIndex) => {
                                        ...
                                        return (
                                            <div key={moonIndex} className={`celestial-body moon ${moonStatus}`} style={{
                                                ...
                                                {/* Etiqueta da Lua (agora herda .body-label e usa .moon-label como modificador) */}
                                                {/* Container da Label movido para fora para facilitar posicionamento */}
                                                <div className="label-container">
...
export default MapaEstelarPlan;